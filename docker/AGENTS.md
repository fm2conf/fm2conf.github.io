# AGENTS.md — Docker Compose SPL Configurator

This is the agent instructions file used to generate the `docker/` configurator in this repository.

The original source of this file is `~/DockerSettings/AGENTS.md`. The instructions below were used with the prompt template from the repository root README:

```
Follow AGENTS.md. Create a configurator for Docker Compose. Here is more information: <docker compose / service configuration files>.
```

The published artefact in this folder is the static `dist/` build output of the generated React + Vite project.
...
Afterwards create **3 distinct views**, each letting the user **select and deselect modules** from a different strategic or visual angle. Every view must produce or contribute to a configuration state — not just display data. Every view is a complete, standalone implementation -- not a shared component reused 3 times.
...
- **Variation required** across the 3 views: different UI libraries, different CSS approaches, different charting or layout libraries. Do not use the same library for every view.
...
- [ ] All 3 views are reachable from the app's main navigation.
...
- [ ] The 3 views are visibly and structurally distinct in layout, component choice, and colour palette.
...
- Producing 3 views that are essentially the same layout with different colours or fonts.
...
- Don't do each view alone. Use subagents
